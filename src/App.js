    ...
    return (
        <div className="app-container">
                            ...
                            <>
                                ...
                                {currentCatName === "Trang chủ" && articles.length > 0 && (
                                    <div className="mb-5">
                                        ...
                                        <Row>
                                            <Col md={7}><Card className="border-0 shadow-sm h-100 cursor-pointer" onClick={() => crawlArticle(articles[0])}><div className="ratio ratio-4x3 bg-light overflow-hidden rounded">{articles[0].imageUrl ? <img src={articles[0].imageUrl} className="object-fit-cover" alt="" /> : <div className="d-flex align-items-center justify-content-center h-100"><i className="bi bi-card-image fs-1 text-muted"></i></div>}</div><Card.Body className="px-0"><Card.Title className="fw-bold fs-3 hover-blue mt-2">{articles[0].title}</Card.Title><Card.Text className="text-muted small">{articles[0].cleanDesc}</Card.Text></Card.Body></Card></Col>
                                            <Col md={5}>{articles.slice(1, 6).map((item, idx) => (<div key={idx} className="d-flex mb-3 pb-3 border-bottom align-items-start cursor-pointer" onClick={() => crawlArticle(item)}><div className="flex-shrink-0 ratio ratio-16x9 bg-light rounded overflow-hidden me-2" style={{ width: '120px' }}>{item.imageUrl ? <img src={item.imageUrl} className="object-fit-cover" alt="" /> : <i className="bi bi-image m-auto text-muted"></i>}</div><h6 className="fw-bold small hover-blue mb-0 line-clamp-3">{item.title}</h6></div>))}</Col>
                                        </Row>
                                    </div>
                                )}
                                {/* LIST TIN THƯỜNG */}
            ...
            <Modal show={!!selectedArticle} onHide={() => setSelectedArticle(null)} size="lg" centered scrollable>
                ...
                <Modal.Footer className="border-0"><Button variant="secondary" onClick={() => setSelectedArticle(null)}>Đóng</Button><Button variant="danger" href={selectedArticle?.link} target="_blank">Xem gốc</Button></Modal.Footer>
            </Modal>
        </div>
    );
}
