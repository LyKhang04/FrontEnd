...
// Danh sách các danh mục hiển thị khối ở dưới
const HOME_BLOCKS = [
...
function App() {
    const [articles, setArticles] = useState([]); // Tin chính
    const [homeBlockArticles, setHomeBlockArticles] = useState({}); // Tin từng khối
    ...
    const crawlArticle = async (article) => {
        ...
        try {
            ...
            if (content) {
                ...
                setDetailContent("<p class='text-center py-3'>Không thể bóc tách. <a href='"+article.link+"' target='_blank'>Xem gốc</a></p>");
            }
        } catch (err) {
        ...
        return (
            <div className="news-section-block mb-5">
                <div className="d-flex justify-content-between align-items-center border-bottom border-danger border-2 mb-3">
                    <h4 className="section-header-title text-danger fw-bold mb-0 text-uppercase py-1 cursor-pointer" onClick={onTitleClick}>{title}</h4>
                        ...
                        {data[0] && (
                            <Card className="border-0 shadow-sm h-100 main-block-card cursor-pointer" onClick={() => crawlArticle(data[0])}>
                                <div className="ratio ratio-16x9 bg-light overflow-hidden rounded">
                                    {data[0].imageUrl ? <img src={data[0].imageUrl} className="object-fit-cover" alt="" /> : <i className="bi bi-image fs-1 m-auto text-muted"></i>}
                                </div>
                        ...
                        <div className="sub-news-list">
                            {data.slice(1, 5).map((item, idx) => (
                                <div key={idx} className="d-flex mb-3 pb-3 border-bottom align-items-start cursor-pointer" onClick={() => crawlArticle(item)}>
                                    ...
                                    <div className="flex-grow-1"><h6 className="fw-bold mb-1 small hover-blue line-clamp-2">{item.title}</h6></div>
                                </div>
                            ))}
                        </div>
                    </Col>
        ...
        <div className="app-container">
                ...
                <Container className="d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div className="d-flex align-items-center header-right">
                        ...
                        <div className="sub-logo-box text-end lh-1 border-start ps-3">
                            <div className="text-danger fw-bold fs-5">GIÁO DỤC</div><div className="text-dark fw-bold small">THỦ ĐÔ</div>
                        </div>
                    </div>
                </Container>
            </header>
                            ...
                            {CATEGORY_TREE.slice(1, 14).map((item, i) => (
                                item.children ? (
                                    <NavDropdown key={i} title={item.name} id={`nav-${i}`} className="custom-dropdown text-white fw-bold text-uppercase">
                                        ...
                                        <NavDropdown.Divider />
                                        {item.children.map((child, idx) => (<NavDropdown.Item key={idx} onClick={() => fetchRSS(child.url, child.name)}>{child.name}</NavDropdown.Item>))}
                                    </NavDropdown>
            ...
            <Container className="mt-4">
                ...
                <Row>
                    {/* --- CỘT CHÍNH (9) --- */}
                    <Col lg={9}>
                        {/* A. PHẦN "TOÀN CẢNH - SỰ KIỆN" (Chỉ hiện ở Trang chủ) */}
                        {currentCatName === "Trang chủ" && !loading && articles.length > 0 && (
                            <div className="mb-5">
                                <div className="toan-canh-title mb-3 d-flex align-items-center">
                                    <h4 className="fw-bold text-danger m-0">Toàn cảnh - Sự kiện</h4>
                                    ...
                                    <Col md={7}>
                                        <Card className="border-0 shadow-sm h-100 cursor-pointer" onClick={() => crawlArticle(articles[0])}>
                                            <div className="ratio ratio-4x3 bg-light overflow-hidden rounded">
                                                {articles[0].imageUrl ? <img src={articles[0].imageUrl} className="object-fit-cover" alt="" /> : <i className="bi bi-card-image fs-1 m-auto text-muted"></i>}
                                            </div>
                                            <Card.Body className="px-0">
                                                <Card.Title className="fw-bold fs-3 hover-blue">{articles[0].title}</Card.Title>
                        ...
                        {/* Nếu không phải trang chủ thì hiện list tin thường */}
                        ...
                        {/* B. CÁC KHỐI DANH MỤC KHÁC (Chỉ hiện ở Trang chủ) */}
                        {currentCatName === "Trang chủ" && HOME_BLOCKS.map((block, idx) => (
                            <NewsSection key={idx} title={block.name} data={homeBlockArticles[block.name] || []} onTitleClick={() => fetchRSS(block.url, block.name)} />
                        ))}
                    </Col>
                    ...
                    <Col lg={3}>
                        ...
                        <div className="sidebar-box mb-4">
                            ...
                            <div className="sidebar-content border border-top-0 p-2 bg-white">
                        ...
                        <div className="sidebar-box mb-4">
                            ...
                            <div className="sidebar-content p-3 bg-light">
                                ...
                                <p className="small text-muted mb-0">GD&TĐ - Ngày 21/3/1978, Thủ tướng Chính phủ ban hành Quyết định số 164...</p>
                            </div>
                        </div>
                        ...
                        <div className="baoin-banner text-center text-white p-4 rounded shadow-sm d-flex flex-column align-items-center justify-content-center cursor-pointer mb-4">
                            ...
                            <h5 className="fw-bold mb-0">ĐỌC BÁO IN</h5><h5 className="fw-bold">ONLINE</h5>
                        </div>
                    </Col>
                </Row>
            </Container>

            {/* Footer & Modal (Giữ nguyên) */}
            <footer className="footer-site mt-5 pt-5 pb-3 text-white" style={{ backgroundColor: '#c92127' }}>
                <Container>
                    <Row className="mb-4">
                        ...
                        <Col md={4} className="mb-4"><h6 className="fw-bold text-uppercase border-bottom border-white border-opacity-25 pb-2 mb-3">Thông tin liên hệ</h6><ul className="list-unstyled small opacity-75 lh-lg"><li><i className="bi bi-geo-alt-fill me-2"></i> 15 Hai Bà Trưng, Hoàn Kiếm, Hà Nội</li><li><i className="bi bi-telephone-fill me-2"></i> 024.3936 9800</li><li><i className="bi bi-envelope-fill me-2"></i> gdtddientu@gmail.com</li></ul></Col>
                        <Col md={3} className="mb-4"><h6 className="fw-bold text-uppercase border-bottom border-white border-opacity-25 pb-2 mb-3">Mạng xã hội</h6><div className="d-flex gap-3 fs-4"><i className="bi bi-facebook cursor-pointer"></i><i className="bi bi-youtube cursor-pointer"></i><i className="bi bi-tiktok cursor-pointer"></i></div></Col>
                    </Row>
