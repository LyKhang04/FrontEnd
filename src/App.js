...
function App() {
    ...
    const cleanDescription = (description) => {
        ...
        return description.replace(/<[^>]*>?/gm, '').substring(0, 200) + "...";
    };
        ...
        <div className="app-container">
            ...
            <div className="top-info-bar bg-light border-bottom py-1">
                <Container className="d-flex justify-content-start align-items-center small text-secondary flex-wrap">
                    ...
                    <span className="me-3 border-start ps-3"><i className="bi bi-telephone me-1"></i> Hotline: <strong className="text-danger">096.733.5089</strong></span>
                    <span className="border-start ps-3"><i className="bi bi-envelope me-1"></i> Email: <span className="text-primary">gdtddientu@gmail.com</span></span>
                </Container>
            </div>
                ...
                <Container>
                    ...
                    <Navbar.Collapse id="main-navbar">
                        <Nav className="w-100 justify-content-between align-items-center">
                            ...
                            {/* Duyệt qua mảng data để tạo Menu */}
                            {CATEGORY_TREE.slice(1, 14).map((item, i) => {
                                // Nếu có mục con (như Giáo dục, Thời sự) -> Tạo Dropdown
                                if (item.children && item.children.length > 0) {
                                    return (
                                        <NavDropdown
                                            ...
                                            title={item.name}
                                            id={`nav-dropdown-${i}`}
                                            className="custom-dropdown text-white fw-bold text-uppercase"
                                        >
                                            <NavDropdown.Item onClick={() => fetchRSS(item.url, item.name)} className="fw-bold text-danger bg-light">
                                                Tất cả {item.name}
                                            </NavDropdown.Item>
                                            <NavDropdown.Divider />
                                            {item.children.map((child, idx) => (
                                                <NavDropdown.Item key={idx} onClick={() => fetchRSS(child.url, child.name)}>
                                                    {child.name}
                                                </NavDropdown.Item>
                                            ))}
                                        </NavDropdown>
                                ...
                                // Nếu không có con -> Tạo Link thường
                                return (
                                    <Nav.Link key={i} className="text-white fw-bold px-2 py-3 nav-link-custom" onClick={() => fetchRSS(item.url, item.name)}>
                                        {item.name}
                                    </Nav.Link>
                                );
                            })}
                                    ...
                                    <Col md={idx === 0 ? 12 : 4} key={idx} className="mb-4">
                                        <Card className={`news-card h-100 border-0 shadow-sm ${idx === 0 ? 'featured-card' : ''}`}>
                                            <Card.Body className={idx === 0 ? 'd-md-flex p-0' : 'p-3'}>
                                                {idx === 0 && (
                                                    <div className="featured-img-box col-md-6 bg-light d-flex align-items-center justify-content-center overflow-hidden">
                                                        {item.imageUrl ? <img src={item.imageUrl} className="w-100 h-100 object-fit-cover" alt="" /> : <i className="bi bi-card-image fs-1 text-muted"></i>}
                                                    </div>
                                                )}
